...
#### api (crates/api)
HTTP API service providing REST endpoints for environment CRUD. Provisions vclusters via helm,
creates Kubernetes namespaces via kube-rs, and applies kustomize overlays via kubectl.

### Repositories

#### EnvironmentRepository (crates/shared/src/repositories/environment/)
Stores environment records (id, name, repos, namespace, status, created_at) in SQLite.
- `mod.rs` — trait + `Environment` struct
- `sqlite.rs` — production implementation using sqlx + SqlitePool
- `mock.rs` — re-exports `MockEnvironmentRepository` generated by `#[mockall::automock]`
...
##### EnvironmentService (crates/shared/src/services/environment.rs)
Validates environment names (lowercase alphanum + hyphens, 1–63 chars), generates UUIDs, and
delegates persistence to `EnvironmentRepository`. Does NOT call K8s — that happens in API handlers.
100% unit test coverage enforced.

#### Repositories

##### environment (crates/shared/src/repositories/environment/)
SQLite-backed repository for environments. See above.

---

## Mahakam-specific notes

### crates/api/src/k8s/
Infrastructure glue for Kubernetes provisioning (namespaces via kube-rs, vcluster via helm,
kustomize overlays via kubectl). Excluded from unit test coverage requirement — integration-tested
via a real cluster only.

### environments/base/
Kustomization base template. Mahakam renders a per-environment overlay at runtime pointing at
this base. Do **not** add env-specific content here; the API generates overlays in temp dirs.
...
### frontends/web/
Nuxt 3 web UI for environment management. Proxies `/backend/**` to the API server.
Unit tests run with vitest; 100% coverage enforced on `api/`, `composables/`, `components/`, `stores/`.

### Namespace labels
Every environment namespace is labeled `mahakam.io/managed=true`. Never delete labeled namespaces
